...
export default function LineChartIllustration() {
	...
	const dataIds = useMemo(
		() =>
			brandLineData.map((_, idx) => ({
				gray: `gray-point-${idx}`,
				brand: `brand-point-${idx}`,
				vertical: `vertical-${idx}`,
			})),
		[],
	);
	...
	return (
		<div className="fade-y relative -mx-32 flex items-center justify-center pt-8">
			...
			>
				<title id={chartTitleId}>Performance comparison line chart</title>
				{/* Gradient definitions */}
				<defs>
					{/* Blue gradient */}
					<linearGradient id={brandGradientId} x1="0" y1="0" x2="0" y2="1">
						<stop
							offset="0%"
				...
				{grayLineData.map((point, index) => (
					<circle
						key={dataIds[index]?.gray ?? `gray-point-${index}`}
						cx={point.x}
						cy={point.y}
				...
				{brandLineData.map((point, index) => (
					<circle
						key={dataIds[index]?.brand ?? `brand-point-${index}`}
						cx={point.x}
						cy={point.y}
						r={hoveredIndex === index ? 5 : 3}
						fill="currentColor"
				...
				{brandLineData.map((point, index) => (
					<g key={dataIds[index]?.vertical ?? `vertical-${index}`}>
						{/* Dashed line */}
						<line
							x1={point.x}
							y1={chartTop}
						...
						>
							<button
								type="button"
								aria-label={`Highlight data at ${timestamps[index]}`}
