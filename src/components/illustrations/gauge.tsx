...
export default function Gauge({
	...
	return (
		<div className="relative flex flex-col items-center">
			<button
				type="button"
				aria-label="Gauge score display"
				className="glass-5 dark:border-brand/20 outline-border/30 dark:outline-background/30 relative z-2 rounded-full p-1 outline-4 transition-transform duration-200 hover:scale-105 focus-visible:outline-2 focus-visible:outline-brand"
				onMouseEnter={() => scoreOnHover !== undefined && setIsHovered(true)}
				onMouseLeave={() => setIsHovered(false)}
				onFocus={() => scoreOnHover !== undefined && setIsHovered(true)}
			...
			>
				<svg
					...
					className="-rotate-90"
					role="img"
					aria-labelledby={gaugeTitleId}
				>
					<title id={gaugeTitleId}>
						{`Gauge score ${displayScore} out of 100`}
					</title>
					<circle
						cx={center}
						cy={center}
			...
			<div className="via-foreground/10 dark:via-brand-foreground/15 absolute -top-[500%] left-1/2 h-[1000%] w-px -translate-x-1/2 bg-linear-to-b from-transparent from-20% to-transparent to-90%" />
		</div>
	);
}
