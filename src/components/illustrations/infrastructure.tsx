...
import LaunchUILogo from "@/components/logos/launch-ui";
...
function InfrastructureIllustration({
	...
	const statusGridId = React.useId();
	const complianceGridId = React.useId();
	const statusCells = React.useMemo(
		() => Array.from({ length: 8 }, (_, idx) => `${statusGridId}-cell-${idx}`),
		[statusGridId],
	);
	const complianceRows = React.useMemo(
		() =>
			Array.from({ length: 8 }, (_, idx) => `${complianceGridId}-row-${idx}`),
		[complianceGridId],
	);
