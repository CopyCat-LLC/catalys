...
export default function ScoresIllustration() {
	...
	return (
		<fieldset
			aria-label="Performance scores illustration"
			className="relative flex h-full w-full items-center justify-center px-4 focus:outline-none"
			onMouseEnter={() => setIsHovered(true)}
			onMouseLeave={() => setIsHovered(false)}
			onFocusCapture={() => setIsHovered(true)}
			onBlurCapture={() => setIsHovered(false)}
		>
			<legend className="sr-only">Performance scores illustration</legend>
